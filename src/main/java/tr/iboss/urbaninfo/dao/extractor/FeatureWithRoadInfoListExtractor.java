...
public class FeatureWithRoadInfoListExtractor implements ResultSetExtractor<List<Feature>> {
	@Override
	public List<Feature> extractData(ResultSet rs) throws SQLException, DataAccessException {
		...
		while (rs.next()) {
			...
			feature.setProperties(propertySetter.propertyWithRoadSetter(rs));
